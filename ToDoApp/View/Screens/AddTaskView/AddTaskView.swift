...
class AddTaskView: UIView {
    ...
    private func configureTitle() {
        titleField.placeholder = "Enter task name"
    ...
    @objc func addList() {
        ...
        let model = TaskModel(id: UUID().uuidString, title: title, icon: selectedIcon, done: false, createdAt: .now)
        delegate?.addTask(model)
    }
}
